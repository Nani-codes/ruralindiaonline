...
const New = dynamic(() => import('@/common/Home/components/New'), { ssr: false });
...
export default function Home({ data, ln_styles }: any) {
	...
	// console.log(pariInformation, thisWeekOnPari, pariMovableSections, monthly, week_days);
	useEffect(() => {
		if (ln_styles) {
	...
	return (
		<>
			...
			<div style={{ maxWidth: '1536px', margin: '0 auto' }}>
				{pariInformation && monthly && week_days ? (
					<TopSession
...
const fetchPariInformation = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				pariInformation: {
		...
		const url = `${BASE_URL}api/home-page?${qs.stringify(query)}&locale=${locale}&publicationState=${isDraft}`;
...
const fetchThisWeekOnPari = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				thisWeekOnPari: {
		...
		const response = await axios.get(
			`${BASE_URL}api/home-page?${qs.stringify(query)}&locale=${locale}&publicationState=${isDraft}`,
		);
...
const fetchPariMovableSections = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
...
const fetchPariRecommends = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				pari_movable_sections: {
...
const fetchPariLibrary = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				pari_movable_sections: {
...
const fetchAudioVideoStories = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				pari_movable_sections: {
...
const fetchMonths = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				Months: {
...
const fetchWeekDays = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				week_days: {
...
const fetchLanguageStyles = async (locale: string, isDraft: string) => {
	try {
		const query = {
			populate: {
				xs: { populate: '*' },
		...
		const response = await axios.get(
			`${BASE_URL}api/language-style?${qs.stringify(query)}&locale=${locale}&publicationState=${isDraft}`,
		);
		return response?.data?.data?.attributes || null;
	} catch (error) {
...
// 	const isDraft = query.type === 'home-page' ? 'preview' : 'live';

// 	try {
// 		const [pariInfo, thisWeekOnPari, movableSections, months, weekDays, pariRecommends, pariLibrary, audioVideoStories, lnStyles] =
// 			await Promise.all([
// 				fetchPariInformation(query.locale, isDraft),
// 				fetchThisWeekOnPari(query.locale, isDraft),
// 				fetchPariMovableSections(query.locale, isDraft),
// 				fetchMonths(query.locale, isDraft),
// 				fetchWeekDays(query.locale, isDraft),
// 				fetchPariRecommends(query.locale, isDraft),
// 				fetchPariLibrary(query.locale, isDraft),
// 				fetchAudioVideoStories(query.locale, isDraft),
// 				fetchLanguageStyles(query.locale, isDraft),
// 			]);

// 		const response = await axios.get(
// 			`${BASE_URL}api/home-page?&locale=${locale}&_publicationState=${isDraft}&populate=pari_movable_sections`,
// 		);
...
// 		const orderedArr2 = movableSectionOrder
// 			.map((item1: any) => {
// 				return presentOrder.find((item2: any) => item2?.__component === item1?.__component);
// 			})
// 			.filter(Boolean); //;
...
export async function getServerSideProps({ locale, query }: any) {
	// Determine the publication state based on query parameters
	// If the type is 'home-page', set state to 'preview'; otherwise, 'live'
	const isDraft = query.type === 'home-page' ? 'preview' : 'live';

	try {
		// Use Promise.allSettled to fetch data concurrently and handle both resolved and rejected promises
		const results = await Promise.allSettled([
			fetchPariInformation(query.locale, isDraft), // Fetch Pari information
			fetchThisWeekOnPari(query.locale, isDraft), // Fetch "This Week on Pari" content
			fetchPariMovableSections(query.locale, isDraft), // Fetch movable sections
			fetchMonths(query.locale, isDraft), // Fetch month data
			fetchWeekDays(query.locale, isDraft), // Fetch weekday data
			fetchPariRecommends(query.locale, isDraft), // Fetch Pari recommends content
			fetchPariLibrary(query.locale, isDraft), // Fetch Pari library content
			fetchAudioVideoStories(query.locale, isDraft), // Fetch audio/video stories
			fetchLanguageStyles(query.locale, isDraft), // Fetch language styles
		]);

		// Extract results, defaulting to null for rejected promises
		const [pariInfo, thisWeekOnPari, movableSections, months, weekDays, pariRecommends, pariLibrary, audioVideoStories, lnStyles] =
			results.map((result) => (result.status === 'fulfilled' ? result.value : null));

		// Fetch the order of movable sections from the API
		const response = await axios.get(
			`${BASE_URL}api/home-page?&locale=${locale}&publicationState=${isDraft}&populate=pari_movable_sections`,
		);

		// Reorder the movable sections based on the API response
		const movableSectionOrder = response?.data?.data?.attributes?.pari_movable_sections || [];
		const presentOrder = [movableSections, pariRecommends, pariLibrary, audioVideoStories];
		const orderedArr2 = movableSectionOrder
			.map((item1: any) => {
		...
		return {
			props: {
				...
				messages: require(`@/locales/${locale}.json`), // Localization messages
				data: {
					pariInfo: pariInfo || {}, // Provide a default empty object if the API call failed
					thisWeekOnPari: thisWeekOnPari || {},
					pari_movable_sections: orderedArr2,
					months: months || {},
					weekDays: weekDays || {},
				},
				ln_styles: lnStyles || {}, // Provide default styles if API call failed
				headers: {
					'Cache-Control': 'no-store', // Disable caching for dynamic data
				},
			},
		};
	} catch (error) {
