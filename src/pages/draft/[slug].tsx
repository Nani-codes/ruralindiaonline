...
import CustomCarousel from '@/pages/article/components/RelatedArticle';
...
export default function Article({ data, republish, donate, messages, ln_styles, categories, alignment_spacing }: any) {
	...
	return (
		<div>
			...
			<div
				style={{
					maxWidth: '1536px', // Assuming 'max-width-container' sets a max-width of 1000px
					margin: '0 auto', // Assuming 'supreme' centers the container
					backgroundColor: 'red', // From 'bg-red'
					paddingBottom: '56px', // From 'pb-[56px]'
					display: 'unset',
				}}>
...
export async function getServerSideProps({ locale, query }: any) {
	...
	const res = await axios({
		url: `${BASE_URL}api/articles?locale=all&filters[slug][$eq]=${slug?.replaceAll(
			'&',
			'%26',
		)}&publicationState=preview&populate=deep,6`,
		method: 'GET',
		headers: {
	...
	// console.log(
	// 	`${BASE_URL}api/articles?locale=all&filters[slug][$eq]=${slug?.replaceAll(
	// 		'&',
	// 		'%26',
	// 	)}&publicationState=preview&populate=deep,6`,
	// );

	const url2 = `${BASE_URL}api/articles?locale=all&filters[slug][$eq]=${slug?.replaceAll(
		'&',
		'%26',
	)}&publicationState=preview&populate[categories]=deep`;
